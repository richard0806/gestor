...
//creación o instanciamiento de un objeto de la Clase Connection
	...
	if($cadena > 0 ){
		...
		*/
		$queryExist = "SELECT id, comentarios FROM tbl_salida WHERE ot LIKE '%{$_POST['OT']}%' LIMIT 1";
		...
		if($exist > 0){
			...
			$id = $fetch_array[0];
			//actualizamos el campo comentarios si este posee valores
			$comentarios = $_POST[comentarios];
				if(!is_null($comentarios) | !empty($comentarios)){
					$query = "UPDATE tbl_salida SET comentarios = comentarios +' / '+ {$comentarios} WHERE id = {$id}";
					mysqli_query($query);
				}
			...
			for ($i=0; $i < count($DATA); $i++) {
				...
				$q[$i] = "INSERT INTO tbl_sdetalles VALUES ('', '{$id}', '".$DATA[$i]->id."','".$DATA[$i]->cantidad."', '".$DATA[$i]->almacen."', '{$now}', '".$DATA[$i]->condicion."')";
				$insert = mysqli_query($con,$q[$i]);
				$objBit->movimientos($con, 'SAL', $_POST['OT'], $DATA[$i]->id, $DATA[$i]->cantidad, $_SESSION['iduser'], $now);
				$objProd->update_alm($con,$DATA[$i]->id,$DATA[$i]->almacen,'sal',$DATA[$i]->cantidad);
				$contador++;
			}
			if($cadena = $contador){
				$respuesta = 'DONE';
			...
			$query = "INSERT INTO tbl_salida VALUES ('','{$_POST['TipoFactura']}','{$_POST['AT']}','{$_POST['OT']}','{$_POST['fecha']}', '{$_POST['comentarios']}' , '{$_SESSION['iduser']}','{$now}','','')";
